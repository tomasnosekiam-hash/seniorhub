...
- **Údržba UI (Senior dashboard)**: overlaye v `ui/HomeOverlays.kt`; dashboard v `ui/HomeDashboard.kt` (**M3**, breakpoint **840 dp**); route + kiosk pinning v `ui/HomeScreen.kt` — viz `docs/changes/2026-04-23-android-matej-postwake-dashboard-m3-responsive.md`; plán refaktoru v `docs/changes/2026-04-18-home-screen-refactor-plan.md`.
...
- **Hybrid (produktový záměr, až bude hlas znovu v produktu)**: Nano vs cloud (Flash) — archiv rozborů v `docs/changes/2026-05-02-matej-cloud-companion-voice-billing-direction.md`, `2026-05-04-matej-cloud-gemini-flash-tts-android.md`, `2026-05-03-matej-conversation-neural-tts-implementation-plan.md`, `2026-05-01-matej-mvp-ai-quality-bar-nano-flash.md`, `2026-04-20-matej-postwake-skills-natural-speech-plan.md`.
...
| `incidents` + zápis při nouzi (tablet) | Plánováno | P1 | Schéma v rules může zůstat; tablet z hlasové cesty **nezapisuje**. FCM při novém incidentu: **`notifyAdminsOnDeviceIncident`** — viz `docs/changes/2026-04-16-android-cloud-remove-matej-voice-ai.md`. |
| Cloud Functions: odeslání FCM při novém vzkazu / incidentu | Hotovo (základ) | P0 | `notifyTabletOnNewMessage` + **`notifyAdminsOnDeviceIncident`**; kanál incidentů (`emergency_incidents`) — `docs/changes/2026-04-17-fcm-emergency-channel-functions-maint.md`. |
...
| **Hlasem vytočit** hovor (asistent / rozpoznání kontaktu) | Plánováno | P0 | V aplikaci není hlasový asistent; budoucí wake + NLU. |
...
- **Cloud Functions**: nasadit `firebase/functions` (`notifyTabletOnNewMessage`, `notifyAdminsOnDeviceIncident`); po deployi lze v konzoli smazat legacy funkce `notifyAdminsOnMatejIncident` a `matej*` callables, pokud tam ještě visí
