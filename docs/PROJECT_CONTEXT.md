...
- **Cloud Functions**: `notifyTabletOnNewMessage` nasazená (FCM na tablet při vzkazu od rodiny; u zpráv od tabletu bez FCM kopie správcům); **`notifyAdminsOnMatejIncident`** — FCM správcům při zápisu `devices/.../incidents` (Matěj / nouze); **oddělený Android `channelId`** pro nouzi vs. vzkazy; moduly `fcmConstants` / `adminFcmTokens`; **Node 22** + **`firebase-functions` v7** — viz `docs/changes/2026-04-17-fcm-emergency-channel-functions-maint.md` (dřívší popis Matěje: `docs/changes/2026-04-16-android-matej-incidents-porcupine-fcm.md`).
- **Matěj (Senior / kiosk) — částečně hotovo**: foreground služba, wake **Porcupine** (volitelně vlastní `.ppn`/`.pv` v assets) nebo **STT** fallback; po probuzení **druhé STT** a heuristická klasifikace příkazu (`matej/MatejPostWake.kt`) — počasí (Open-Meteo), čtení nepřečteného vzkazu z webu (`fetchFirstUnreadFamilyMessage`), diktát SMS (druhé STT + cellular / Firestore jako dashboard), hovor (klíčová slova + kontakt nebo jen jméno), jinak **nouze** (`incidents` + FCM). Debug nouze bez wake: `docs/changes/2026-04-19-android-home-dashboard-matej-debug-test.md`. Detail a **plán vrstvy přirozené řeči** (intent, Nano/AICore vs cloud vs heuristiky): `docs/changes/2026-04-20-matej-postwake-skills-natural-speech-plan.md`.
...
- **Údržba UI (probíhá)**: overlaye v `ui/HomeOverlays.kt`; dashboard v `ui/HomeDashboard.kt`; route + kiosk pinning v `ui/HomeScreen.kt` — `docs/changes/2026-04-19-android-home-dashboard-matej-debug-test.md`; plán v `docs/changes/2026-04-18-home-screen-refactor-plan.md`.
...
- **Emergency Voice Proxy (modul „Matěj“)** (pouze **režim Senior / klientský tablet**): základ v repu — wake (Porcupine / STT), nouzové vytočení, TTS, zápis `incidents` a FCM pro správce — viz `docs/changes/2026-04-16-android-matej-incidents-porcupine-fcm.md`; rozšíření o dovednosti po probuzení a roadmapa **přirozené řeči / intentů** — `docs/changes/2026-04-20-matej-postwake-skills-natural-speech-plan.md`; hlasitý odposlech, širší reporting — dál (`oldies_project.md`).
- **Push – Firebase Cloud Messaging (FCM)**: **na Androidu** rozlišené **notifikační kanály** — vzkazy (`family_messages`) vs. **nouze / Matěj** (`emergency_incidents` u správců při incidentu); tablet dál vzkazy přes `family_messages`. Odesílání z cloudu přes **Cloud Functions** v `firebase/functions` — viz `docs/changes/2026-04-17-fcm-emergency-channel-functions-maint.md`.
...
- **Gemini Nano (AICore)**: lokální běh přes systémové komponenty Google tam, kde je OEM dodá – **u ne-Pixel tabletů často nejde spolehlivě ověřit před doručením** (žádný úplný veřejný seznam). Po rozbalení: aktualizace systému a Google aplikací, kontrola dostupnosti **AI Core** / nastavení AI v systému. **Cíl:** kde Nano poběží, využít ho zejména pro **základní komunikaci a mapování přirozené řeči na záměr** (intent); přesné chování se ověří na referenčním tabletu.
- **Hybrid**: aplikace by měla umět stejný typ úkolu obsloužit přes **cloud Gemini** (např. Flash/Pro / API podle produktu), když Nano na zařízení **není** nebo **nestačí** – viz také hybridní model v `oldies_project.md` (Nano lokálně + cloud pro složitější konverzaci). **Heuristiky** (aktuální klasifikace v `MatejPostWake.kt`) zůstávají jako **fallback** (offline, predikovatelná nouze) i po zavedení modelu — viz `docs/changes/2026-04-20-matej-postwake-skills-natural-speech-plan.md`.
...
| **Matěj**: poslouchat, vytáčet, psát zprávy, číst zprávy, hlasitý odposlech, počasí | Sekce Matěj + tabulka níže | Částečně – nouze + wake + **dovednosti po probuzení** (heuristiky: počasí, vzkaz z webu, SMS, hovor; ne plné NLU) — `docs/changes/2026-04-20-matej-postwake-skills-natural-speech-plan.md`; **plná přirozená řeč** přes Nano/cloud intent — plán v téže změně; repro u hovoru — dál. |
