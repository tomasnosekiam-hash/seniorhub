...
- **Cloud Functions**: `notifyTabletOnNewMessage` nasazená (FCM na tablet při vzkazu od rodiny; u zpráv od tabletu bez FCM kopie správcům); **`notifyAdminsOnMatejIncident`** — FCM správcům při zápisu `devices/.../incidents` (historicky vázané na tablet; **Android tablet už tento zápis z hlasové cesty neprovádí** — viz níže); **oddělený Android `channelId`** pro incidenty vs. vzkazy; moduly `fcmConstants` / `adminFcmTokens`; **Node 22** + **`firebase-functions` v7** — viz `docs/changes/2026-04-17-fcm-emergency-channel-functions-maint.md` (dřívší popis zápisu z tabletu: `docs/changes/2026-04-16-android-matej-incidents-porcupine-fcm.md`).
- **Hlasový asistent na tabletu („Matěj“)**: starý modul odstraněn — `docs/changes/2026-04-24-android-remove-matej-voice-module.md`. **Matěj 2.0** — spec: `docs/changes/2026-04-25-matej-2-product-spec.md`; UI overlay: `docs/changes/2026-04-26-android-matej-assistant-ui-milestone.md`; **wake (Porcupine) → TTS → poslech 5 s**: `docs/changes/2026-04-27-android-matej-wake-listen-flow.md`; **relace s heuristikami + vstup z dashboardu** („Spustit Matěje“): `docs/changes/2026-04-28-android-matej-2-heuristics-dashboard.md`; **MatejBrain (Nano pokud AVAILABLE → cloud Gemini → heuristiky), potvrzení SMS/hovoru**: `docs/changes/2026-04-29-android-matej-gemini-brain-confirmation.md`; **ověření na zařízení, TTS vs Flash vs Nano**: `docs/changes/2026-04-30-matej-device-verification-and-ai-clarification.md`; **laťka MVP pro AI**: `docs/changes/2026-05-01-matej-mvp-ai-quality-bar-nano-flash.md`; **směr bez spolehlivého Nano (cloud, konverzace, hlas, účtování)**: `docs/changes/2026-05-02-matej-cloud-companion-voice-billing-direction.md` — hlas dnes **Android TTS** (pro produkt často nedostatečný; směr **neurální TTS**); **Gemini Nano** v kódu přes ML Kit, na řadě OEM **často nedostupné** → reálná cesta **cloud** (výchozí např. **Gemini 3.1 Flash-lite** v `gemini.cloud.model` / Gradle); Logcat filtr **`Matej`** zahrnuje i STT (`MatejVoicePipeline`). Doplňující plán dovedností: `docs/changes/2026-04-20-matej-postwake-skills-natural-speech-plan.md`; referenční tablet: `docs/changes/2026-04-21-tablet-reference-on-hand-verification.md`.
...
- **Údržba UI (Senior dashboard)**: overlaye v `ui/HomeOverlays.kt`; dashboard v `ui/HomeDashboard.kt` (**M3** typografie / barvy / tvary v `ui/theme/`, responzivita breakpoint **840 dp**, jeden vs. dva sloupce); route + kiosk pinning v `ui/HomeScreen.kt` — viz `docs/changes/2026-04-23-android-matej-postwake-dashboard-m3-responsive.md` (M3/responzivita; hlasový asistent už v repu není — `2026-04-24-android-remove-matej-voice-module.md`); plán refaktoru v `docs/changes/2026-04-18-home-screen-refactor-plan.md`.
...
- **Produktová priorita**: **klientská část u seniora** (tablet v **kiosku** – komunikace, přístupnost, vzkazy, počasí; **bez hlasového asistenta v aktuální aplikaci**) je **nejdůležitější vrstva produktu**. Bez ní nemá smysl „správcovská“ strana jako samostatná hodnota.
- **Admin režim (Android)**: správa zařízení, kontaktů, nastavení; žádný hlasový asistent v admin UI (správce ovládá aplikaci klasicky).
...
- **Nouze / incident (hlasový proxy)**: v aktuální aplikaci **není** — viz `docs/changes/2026-04-24-android-remove-matej-voice-module.md`. Historický návrh a zápisy `incidents`: `docs/changes/2026-04-16-android-matej-incidents-porcupine-fcm.md`; roadmapa řeči: `docs/changes/2026-04-20-matej-postwake-skills-natural-speech-plan.md`; produktový směr dál v `oldies_project.md`.
- **Push – Firebase Cloud Messaging (FCM)**: **na Androidu** rozlišené **notifikační kanály** — vzkazy (`family_messages`) vs. **incidenty / nouze** (`emergency_incidents` u správců); tablet dál vzkazy přes `family_messages`. Odesílání z cloudu přes **Cloud Functions** v `firebase/functions` — viz `docs/changes/2026-04-17-fcm-emergency-channel-functions-maint.md`.
...
- **Tablet pro vývoj**: **Lenovo Idea Tab 11 5G** – **MediaTek Dimensity 6300**, **Mali-G57**, **8 GB RAM**, **Android 15**, 5G. Referenční kus **je používán k ladění** (nasazení aplikace, kiosk, síť/SIM dle scénáře); finální podpora dalších zařízení je samostatné rozhodnutí. Milník „tablet u vývojáře“: `docs/changes/2026-04-21-tablet-reference-on-hand-verification.md`.
- **Gemini Nano (AICore)**: lokální běh přes systémové komponenty Google tam, kde je OEM dodá. Na **fyzickém referenčním tabletu** lze ověřit dostupnost a chování (aktualizace systému a Google aplikací, nastavení **AI Core** / AI v systému). U ne-Pixel zařízení zůstává variabilita mezi kusy — výsledky z tohoto tabletu zapisovat do vývoje jako referenční. **Cíl (až bude hlasová vrstva znovu v produktu):** mapování přirozené řeči na záměr (intent).
- **Hybrid**: **Nano** tam, kde AICore/OEM dovolí; jinak **cloud** (typicky **Flash / Flash-lite**). Produktový rozbor (konverzace, společník, paušál a stropy nákladů): `docs/changes/2026-05-02-matej-cloud-companion-voice-billing-direction.md`. **Konverzace vs skript, neurální TTS (Google na zařízení jako první krok), neúčtovat speech zbytečně — implementační plán:** `docs/changes/2026-05-03-matej-conversation-neural-tts-implementation-plan.md`. Laťka modelů: `docs/changes/2026-05-01-matej-mvp-ai-quality-bar-nano-flash.md`; plán v `docs/changes/2026-04-20-matej-postwake-skills-natural-speech-plan.md` (aktuálně **bez** integrovaného asistenta v aplikaci — `docs/changes/2026-04-24-android-remove-matej-voice-module.md`).
- **Priorita UX**: uživatelská přívětivost a přirozená řeč mají přednost; technická vrstva (Nano vs cloud vs pravidla) se **přizpůsobí** tak, aby výsledný zážitek byl co nejlepší (stejný dokument změny 2026-04-20).

## Směr produktového MVP (shrnutí)
- **Klientský tablet (Senior, kiosk)**: základní „produk“ – **kiosk** jako běžný režim a komunikace (hlasový asistent v aktuální aplikaci **není**); admin aplikace tyto funkce nekopíruje.
...
- **Bezpečnost a rodina**: zápisy incidentů / **FCM** (když budou znovu naplněny), stav zařízení pro admina (viz spec v2); hlasový modul na tabletu zatím odstraněn — `docs/changes/2026-04-24-android-remove-matej-voice-module.md`.
...
### Hlasová asistence — požadované schopnosti (produktový cíl, MVP)

**Stav v kódu:** integrovaný asistent **není** — `docs/changes/2026-04-24-android-remove-matej-voice-module.md`. Tabulka popisuje směr, až se vrátí kvalitní wake + NLU.

**Rozsah nasazení (až bude):** **jen režim Senior** (klientský tablet v kiosku). V **režimu Admin** žádný wake / hlasový proxy.

| Schopnost | Poznámka / napojení |
|-----------|---------------------|
| Poslouchat (wake / povel) | Foreground service, lokální wake word (např. Porcupine / OEM). |
...
**Nejzásadnější**: **klientská verze pro seniora** – tablet v **kiosku** (zero UI launcher, odemčení PINem z cloudu, pairing), na něm **komunikace** (dotykem: hovory, SMS, vzkazy; **hlasová vrstva zatím není v aplikaci**). Bez solidního kiosku a senior UX nemá smysl stavět zbytek jako uživatelsky hotový produkt.  
**Admin (Android)**: správa zařízení a dat; paralela s weby správy.  
...
**Balík „bezpečí a zdraví“** (viz níže) až po posunu v kiosku a komunikaci – sladit pravidla času, tísňové kanály a provozní přehled (**kiosk jako takový do tohoto balíku nepatří** – je v Balíku 1).

### Implementační balík 1 – Komunikace a senior tablet
...
| Cloud Functions: odeslání FCM při novém vzkazu / incidentu | Hotovo (základ) | P0 | `notifyTabletOnNewMessage` + **`notifyAdminsOnMatejIncident`**; rozlišení **kanálu** u incidentů (`emergency_incidents`) — `docs/changes/2026-04-17-fcm-emergency-channel-functions-maint.md`; nasazení Matěje/incidentů viz 2026-04-16. |
...
| Seznam „Moje tablety“, párování kódem z tabletu | Hotovo | P0 | UX a idempotence párování: `docs/changes/2026-04-22-pairing-ux-firestore-matej-stt-firebase-cli.md` |
...
## Emergency Voice Proxy (modul „Matěj“) — stav v repu

**Aktuálně (od 2026-04-24):** experimentální implementace hlasového asistenta z aplikace **odstraněna** — viz **`docs/changes/2026-04-24-android-remove-matej-voice-module.md`**. Níže zůstává **produktový návrh / spec** pro případné obnovení vrstvy (wake, Nano, nouze).

**Platilo by pouze pro klientský tablet v režimu Senior (kiosk).** V admin režimu se neprovozuje.

Účel (vize): poskytnout seniorovi **okamžitou hlasovou pomoc** ve fyzické tísně bez ovládání dotykem – kombinace wake wordu, AI kontextu a eskalace hovoru.
...
- `devices/{deviceId}/incidents/{incidentId}` — dříve zápis tabletem z hlasové nouzové sekvence (`source: matej_emergency`); **aktuální Android aplikace už nezapisuje** z této cesty — viz `docs/changes/2026-04-24-android-remove-matej-voice-module.md`. Schéma a FCM pro správce mohou sloužit budoucím zdrojům incidentů; rozšíření o přepis / typy — plánováno.
...
- `firebase/firestore.rules`
  ...
  - pairing je vázaný na existující `pairingClaims/{code}` s expirací a jednorázovým použitím; update claimu po webovém párování přes dotčené klíče (`usedAt` / `usedByUid`) — viz `docs/changes/2026-04-22-pairing-ux-firestore-matej-stt-firebase-cli.md`
  - **Role `viewer`**: čtení zařízení a **vzkazy** (create bez `delivery`); zápis **`contacts`** a **`config`** jen pro tablet nebo ne-viewer správce — viz `docs/changes/2026-04-16-inbound-sms-fcm-roles-web-viewer.md`. **Dokončení párování** (zápis jen `paired` / `pairedAt` na `devices` pro viewer) — `docs/changes/2026-04-22-pairing-ux-firestore-matej-stt-firebase-cli.md`. Android admin app bez zvláštního rozlišení ve rules (plný správce).

## Firebase Console checklist
- **Authentication**: povolit `Google` a `Anonymous`
- **Firestore rules**: publikovat obsah `firebase/firestore.rules`
- **Firebase CLI v kořeni repa**: ujistit se, že aktivní projekt odpovídá `seniorhub-716f0` (`firebase use seniorhub-716f0` nebo `.firebaserc`); špatný záznam v `~/.config/configstore/firebase-tools.json` (`activeProjects` pro tuto složku) umí CLI zmást — viz `docs/changes/2026-04-22-pairing-ux-firestore-matej-stt-firebase-cli.md`
