...
| **Základní info o uživateli** – jméno, příjmení, adresa, **kritické kontakty v nouzi** | Rozšířit `config` / profil zařízení | Doplnit – viz řádky „Profil seniora“ v tabulkách. |
...
- **Doporučené pole** (pod `devices/{deviceId}/config` nebo samostatný dokument): `seniorFirstName`, `seniorLastName`, `address` (strukturovaně nebo jeden řádek), případně `displayName` pro UI.  
- **Kritické kontakty v nouzi**: buď označení v `contacts` (`isEmergency: true`, `sortOrder`), nebo samostatný seznam ID kontaktů v `config` (`emergencyContactIds`).  
- **Admin** tyto údaje mění v **admin režimu** (Android) a/nebo na **webu**; tablet je jen zobrazuje / používá pro Matěje a hovory.
...
| Security Rules (tablet + admin podle vazeb, pairing s expirací) | Hotovo | P0 | Rozšířit o role web vs plný admin → Plánováno. |
| Anonymní Auth (tablet) + Google (správci) | Hotovo | P0 | Konzole: Anonymous + Google. |
| `devices/.../config` (PIN, SIM, jméno asistenta, …) | Plánováno | P0 | Struktura v `oldies_project.md`; zatím část polí na `devices/{id}`. |
| `config`: **profil seniora** (jméno, příjmení, adresa) + reference na **kritické kontakty** | Plánováno | P0 | Viz sekce *Profil seniora a nouzové kontakty*. |
| `devices/.../messages` + potvrzení přečtení | Plánováno | P0 | Nutné pro rodinný web + tablet. |
...
| `devices/.../status` (online, baterie, signál) | Plánováno | P1 | Pro dashboard správce. |
| `incidents` + zápis při nouzi | Plánováno | P1 | Navázat na Matěj / FCM. |
| Cloud Functions: odeslání FCM při novém vzkazu / incidentu | Plánováno | P0 | Bez serveru nejde spolehlivě pushnout ostatním. |
| Ukládání FCM tokenů (zařízení / správci) | Plánováno | P0 | Viz pole typu `fcmTokens` v dokumentaci modelu. |
...
| **Kiosk** – klientský režim jako launcher / „zero UI“, základ dashboardu (hlasitost, alert), odemčení nastavení | Částečně | P0 | **Zásadní** pro produkt; doplnit plné chování kiosku, gesto, **PIN z cloudu** podle spec. |
...
| Skryté gesto + keypad + odemčení vs **admin PIN** ve Firebase | Plánováno | P0 | Viz krok 3 v `oldies_project.md`. |
...
| Seznam kontaktů → **vytočení hovoru** (dotyk) | Plánováno | P0 | Telefonní oprávnění. |
...
| SMS čtení / psaní (minimálně odeslání) | Plánováno | P1 | Silná oprávnění; sloučit s chat UI. |
| **Počasí** – widget na ploše (free API + cache) | Plánováno | P1 | Např. Open-Meteo; sdílet data s Matějem. |
| Příjem **FCM** → fullscreen vzkaz / prioritní UI | Plánováno | P0 | + zápis „přečteno“ do Firestore. |
...
| Dashboard: zobrazení / úprava **PINu** tabletu, **SIM** | Plánováno | P0 | Závisí na `config` dokumentu. |
...
| Úprava nastavení a kontaktů pro vybrané zařízení | Hotovo | P0 | Rozšířit s `config` / rolemi. |
| Vzkazy do `messages` + viditelnost stavu přečtení | Plánováno | P0 | + rules pro „rodina“ vs admin. |
...
6. **Kontakty → hovor** (dotyk), pak **hlasové vytáčení a diktát**; **sjednocený chat** (SMS + in-app) podle kapacity.  
...
- `firebase/` – Firebase projekt v repu: nyní především **`firestore.rules`**; očekávané doplnění: **`functions/`** (odesílání FCM při zápisu do Firestore), případně `firebase.json` při rozšíření
- `docs/` – `PROJECT_CONTEXT.md`, changelog v `docs/changes/`
...
- `devices/{deviceId}/config` – např. `admin_pin`, `sim_number`, `assistant_name`, `paired_admin_id`
